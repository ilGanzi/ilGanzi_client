...
export default function SettingPage() {
  ...
  const handlePasswordInput = (e) => {
    ...
    const isPasswordValid =
      password.length >= 8 &&          // 비밀번호 길이가 8자 이상
      /[a-zA-Z]/.test(password) &&     // 영문자가 1개 이상
      /\d/.test(password) &&           // 숫자가 1개 이상
      /[@$!%*?&]/.test(password);      // 특수문자(@, $, !, %, *, ?, &)가 1개 이상
    ...
    <styles.SettingContainer>
      <styles.SettingWrapper>
        ...
        <styles.PassWord_InputSection>
          ...
          />
          <FontAwesomeIcon icon={faEye} onClick={handleShowIPW} /> {/* 클릭 시 handleShowIPW 호출 */}
        ...
        <styles.PassWord_CheckSection>
          <styles.PassWord_Check
            ...
            type={showCPW ? "text" : "password"} // showCPW 값에 따라 비밀번호 보이기/감추기
            placeholder='비밀번호 재입력'
            style={{
              backgroundColor: 'white',
          ...
          />
          <FontAwesomeIcon icon={faEye} onClick={handleShowCPW} /> {/* 클릭 시 handleShowCPW 호출 */}
          {passwordError && CPW !== "" && (
            <div style={{ color: 'red' }}>
              비밀번호가 일치하지 않습니다.
            </div>
